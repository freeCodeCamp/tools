...
class PrInfo {
  ...
  // `.hasNextPage et al are deprecated`. Use:
  ...
  async getPRs(totalPRs, firstPR, lastPR, prPropsToGet) {
    ...
    let getAll = await
    // TODO get this.github.paginate() working
    // https://probot.github.io/docs/pagination/
    // this.github.paginate(
      this.github.pullRequests.list(
        this.methodProps
      ).then(async(res) => {
        let openPRs = await this.prFilter(
          res.data,
          firstPR, lastPR, prPropsToGet
        );
