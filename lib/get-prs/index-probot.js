...
class Pr {
  ...
  async getRange() {
    ...
    let response = await context.github.pullRequests.list(this.methodProps);
    const firstPR = response.data[0].number;
    this.methodProps.direction = 'desc';
    response = await context.github.pullRequests.list(this.methodProps);
